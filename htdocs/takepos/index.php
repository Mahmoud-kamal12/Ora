...
// Mahmoud Kamal Edit . incre finction too increment value and decr to decrement value
$(document).on('click' , '.incre' , incre)
$(document).on('click' , '.decr' , decr)
function incre() {

    editnumber2 = Number($('tr.selected > #datatoincordec').data('qty')) + 1
    var trid = $('tr.selected').attr('id');

    $("#poslines").load("invoice.php?invoiceid="+$('#invoiceid').val()+"&action=updateqty&place="+place+"&idline="+trid+"&number="+editnumber2, function() {
            editnumber="";
            // $('#poslines').scrollTop($('#poslines')[0].scrollHeight);
            $("#qty").html("<?php echo $langs->trans("Qty"); ?>");

            $('table#tablelines > tbody  > tr').each(function(index, tr) {
...
function decr() {
    ...
    if(Number($('tr.selected > #datatoincordec').data('qty')) <= 0){
        return 0;
    }
    editnumber2 = Number($('tr.selected > #datatoincordec').data('qty')) - 1
    var trid = $('tr.selected').attr('id');

    $("#poslines").load("invoice.php?click=1&invoiceid="+$('#invoiceid').val()+"&action=updateqty&place="+place+"&idline="+trid+"&number="+editnumber2, function() {
        editnumber="";
        //$('#poslines').scrollTop($('#poslines')[0].scrollHeight);
        $("#qty").html("<?php echo $langs->trans("Qty"); ?>");
...
function Edit(number) {
	...
	if (number=='c'){
		...
		if (editaction=='qty' && editnumber!=""){
            // Mahmoud Kamal Edit add paramter invoiceid="+$('#invoiceid')
			
            $("#poslines").load("invoice.php?invoiceid="+$('#invoiceid').val()+"&action=updateqty&place="+place+"&idline="+selectedline+"&number="+editnumber, function() {
                editnumber="";
                //$('#poslines').scrollTop($('#poslines')[0].scrollHeight);
                $("#qty").html("<?php echo $langs->trans("Qty"); ?>");
            });
...
function DirectPayment(){
    ...
    $.ajax({
        type: "GET",
        url: "invoice.php?place="+place+"&action=invoiceType&invoiceid="+$('#invoiceid').val(),
        success: function (response) {
            if (response.type == 2){
                $("#poslines").load("invoice.php?place="+place+"&action=valid&invoiceid="+$('#invoiceid').val()+"&pay=LIQ", function() {
                });
            }
        }
    });
